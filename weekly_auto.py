...
def save_all_data(data):
    """팀원 목록과 계획 전체를 구글 시트에 저장합니다 (팀원 관리, 보고서 삭제 등 큰 변경 시 사용)."""
    members_sheet, plans_sheet = connect_to_gsheet()
    ...
    try:
        ...
        if not all_plans_df.empty:
            row_index_list = all_plans_df.index[
                (all_plans_df['week_id'].astype(str) == str(week_id)) & (all_plans_df['member_name'] == member_name)
            ].tolist()
        if row_index_list:
            plans_sheet.update_cell(row_index_list[0] + 2, 3, plan_json_str)
        else:
...
def generate_pdf(plans_data, members_data, year, week, week_dates, prev_week_dates, day_names, team_order):
    ...
    if not os.path.exists(FONT_FILE):
        st.error(f"PDF 폰트 파일 '{FONT_FILE}'을 찾을 수 없습니다.")
        return None
    pdf = FPDF()
    ...
    for team_name in team_order:
        ...
        if not team_members_in_group or not any(m.get('name') in plans_data for m in team_members_in_group): continue
        ...
        for member_data in team_members_in_group:
            ...
            for i, day_key in enumerate(['mon', 'tue', 'wed', 'thu', 'fri']):
                am, pm = member_plan.get('grid', {}).get(f'{day_key}_am', ''), member_plan.get('grid', {}).get(f'{day_key}_pm', '')
                if am or pm:
            ...
            pdf.set_font('NanumGothic', '', 10)
            for i, day_key in enumerate(['mon', 'tue', 'wed', 'thu', 'fri']):
                am, pm = member_plan.get('lastWeekGrid', {}).get(f'{day_key}_am', ''), member_plan.get('lastWeekGrid', {}).get(f'{day_key}_pm', '')
                if am or pm:
                    pdf.set_font('NanumGothic', 'B', 10)
